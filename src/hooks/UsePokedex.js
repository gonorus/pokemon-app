...
const UsePokedex = () => {
  ...
  useEffect(() => {
    if (!pokedex) {
      ...
      if (localStore)
        setPokedex(JSON.parse(localStore)); // sync local pokedex with current context
      else
        localStorage.setItem('pokedex', JSON.stringify({})); // initialize local pokedex
    }
    else
      localStorage.setItem('pokedex', JSON.stringify(pokedex)); // update local pokedex
  }, [pokedex]);
